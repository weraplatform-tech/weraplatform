...
async function callOpenRouter(messages: any[], model = 'deepseek/deepseek-chat') {
...
matchingRouter.post('/providers', async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const providerList = providers.map(p => ({
      id: p.id,
      name: `${(p.profiles as any).first_name} ${(p.profiles as any).last_name}`,
      title: p.title,
      skills: p.skills?.join(', '),
      rate_kes: p.hourly_rate_kes,
      rating: p.rating_avg,
      jobs: p.jobs_completed,
      county: (p.profiles as any).county,
      verified: (p.profiles as any).id_verified === 'verified',
    }));
...
// ── AI Price Suggestion ───────────────────────────────────────
matchingRouter.post('/price-suggest', async (req: AuthRequest, res) => {
...
matchingRouter.post('/generate-job-description', async (req: AuthRequest, res) => {
