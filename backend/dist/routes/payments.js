...
exports.paymentsRouter.post('/mpesa/stk-push', [
    (0, express_validator_1.body)('booking_id').isUUID(),
    (0, express_validator_1.body)('phone').matches(/^(254|0)[7][0-9]{8}$/),
], async (req, res) => {
    ...
    try {
        ...
        // Initiate STK Push
        const stkResult = await (0, mpesa_1.initiateStkPush)({
            phone,
            amount,
        ...
        const { data: payment } = await supabase_1.supabaseAdmin.from('payments').insert({
            booking_id,
            payer_id: userId,
...
exports.paymentsRouter.post('/mpesa/callback', async (req, res) => {
    ...
    try {
        ...
        if (success) {
            ...
            // Update payment
            await supabase_1.supabaseAdmin.from('payments').update({
                status: 'escrowed',
                mpesa_ref: checkoutId,
                mpesa_receipt: mpesaRef,
            }).eq('id', payment.id);
            // Update booking status
            await supabase_1.supabaseAdmin.from('bookings').update({ status: 'in_progress' }).eq('id', payment.booking_id);
            // Escrow: hold in platform
            await supabase_1.supabaseAdmin.from('wallet_transactions').insert({
                wallet_id: (await supabase_1.supabaseAdmin.from('wallets').select('id').eq('user_id', payment.payer_id).single()).data?.id,
                type: 'escrow_hold',
            ...
            await supabase_1.supabaseAdmin.from('payments').update({ status: 'failed' }).eq('id', payment.id);
        }
        return res.json({ ResultCode: 0, ResultDesc: 'Accepted' });
    }
    catch (err) {
...
exports.paymentsRouter.post('/release-escrow/:booking_id', async (req, res) => {
    ...
    try {
        ...
        if (booking.status !== 'completed')
            return res.status(400).json({ error: 'Booking must be marked complete first' });
        ...
        if (wallet) {
            ...
            await supabase_1.supabaseAdmin.from('wallets').update({
                balance_kes: newBalance,
                total_earned_kes: supabase_1.supabaseAdmin.rpc('increment', { x: payment.net_amount_kes }),
            }).eq('id', wallet.id);
            await supabase_1.supabaseAdmin.from('wallet_transactions').insert({
                wallet_id: wallet.id,
                type: 'escrow_release',
